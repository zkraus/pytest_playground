...
def get_pytest_test_cases() -> list[str]:
    pytest_test_cases = [ x for x in  subprocess.run(["pytest", "--co", "-q"], capture_output=True).stdout.decode("utf-8").splitlines()[:-2]]
...
def filter_existing_test_cases(tree: fmf.Tree, test_cases: list[tuple[str,str]]) -> list[tuple[str, str]]:

    non_existing_test_cases = [ x for x in test_cases if not tree.find(f'/{x[0]}/::{x[1]}') ]
...
def append_non_existing_test_case(test_path, test_name):
    ...
    with open(test_pathname, "a") as f:
        ...
        tc_object = {
            fmf_test_name: {'TODO': 'define metadata'}
        }
